...
//
// RAM A/B MODE SELECT: ram_b_mode (input, see its own port comment
// below) steers the PSRAM front-end's chip-select between CS1 (RAM A,
// default) and CS2 (RAM B - only meaningful once that Pmod trace has
// been cut, see spi_ctrl.v's header). It is latched once at reset in
// the top level and held constant, not a live mode switch. Whichever
// mode is NOT selected for the PSRAM front-end at a given moment, the
// generic SPI front-end (spi_ctrl) is correspondingly blocked from CS2
// in RAM-B mode - see spi_want and the safe no-op completion path
// below for how that's kept from stalling a8_core if firmware gets
// this wrong.
...
module qspi_shared_engine #(
    ...
    // ---- RAM A/B mode select (see STRAP register, tt_um_agila8.v) ----
    // Latched once at reset in the top level, held constant afterward -
    // NOT a live signal. Steers which physical CS pin the psram
    // front-end's transactions go out on (CS1/RAM A when 0, the
    // default; CS2/RAM B when 1, only meaningful after that Pmod
    // trace has actually been cut - see spi_ctrl.v's header). When 1,
    // the generic SPI front-end (spi_ctrl, also CS2) is blocked from
    // ever asserting CS2 - see spi_want below - since that pin now
    // belongs to real RAM B traffic and must not see spurious SPI
    // command bytes.
    input  wire        ram_b_mode,
    ...
    // PSRAM owner drives CS1 in the default mode (ram_b_mode=0), or CS2
    // in RAM-B mode (ram_b_mode=1) - see header note on this port.
    // SPI owner only ever drives CS2, and only in the default mode -
    // spi_want (below) already prevents OWNER_SPI from being granted at
    // all while ram_b_mode=1, so these two conditions are mutually
    // exclusive by construction, not just by the pin mux below.
    assign cs_n_psram = !(cs_active_r && owner_r == OWNER_PSRAM && !ram_b_mode);
    assign cs_n_spi   = !(cs_active_r && (
                            (owner_r == OWNER_PSRAM && ram_b_mode) ||
                            (owner_r == OWNER_SPI   && !ram_b_mode)
                          ));
    ...
    // Gated by !ram_b_mode: while CS2 belongs to real RAM B traffic,
    // the SPI front-end must never be granted a real transfer - see
    // the cs_n_spi note above and the safe-no-op completion path below,
    // which keeps a8_core from stalling forever if firmware writes
    // SPI_DATA in this mode instead of just leaving the request
    // ungranted.
    wire spi_want    = spi_valid && spi_hit && (spi_addr == ADDR_DATA)
                        && spi_we && !just_finished_spi && !ram_b_mode;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // Safe no-op: a DATA write while ram_b_mode=1 would
            // otherwise never be granted (spi_want excludes this case
            // above) and a8_core would stall forever waiting for
            // dmem_ready. Complete it immediately instead, WITHOUT
            // touching cs_n_spi/sck/mosi or the shared shift register -
            // the byte is silently dropped, not sent anywhere. This is
            // the safe failure mode: the alternative (actually driving
            // CS2) would send spurious command bytes to the real RAM B
            // chip now using that pin, corrupting live DMEM data.
            // Software should check STRAP (0xF5) and simply not use
            // SPI_DATA/SPI_CTRL in this mode - this path exists so
            // getting that wrong hangs nothing and corrupts nothing,
            // it just silently does nothing.
            if (spi_valid && spi_hit && spi_addr == ADDR_DATA && spi_we
                && ram_b_mode && !just_finished_spi)
                spi_ready <= 1'b1;
